...
int main()
{
    ...
    cout << "Give the cordinates of the clipping window (xmin ymin xmax ymax): ";
    ...
    {
        int x1, y1, x2, y2;
        cout << "Give inputs for the cordinates of the line " << (i + 1) << " (x1 y1 x2 y2): ";
        cin >> x1 >> y1 >> x2 >> y2;
        setcolor(WHITE);
        ...
        int outcode1 = computeOutCode(x1 + centerX, centerY - y1, xmin + centerX, centerY - ymax, xmax + centerX, centerY - ymin);
        int outcode2 = computeOutCode(x2 + centerX, centerY - y2, xmin + centerX, centerY - ymax, xmax + centerX, centerY - ymin);
    ...
    delay(8000); // Wait for 5 seconds before closing the window
    closegraph();
    return 0;
}
